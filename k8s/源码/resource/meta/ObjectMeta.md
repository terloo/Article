# ObjectMeta
ObjectMeta位于`k8s.io/apimachinery/pkg/apis/meta/v1/types.go`包下，是用于描述k8s资源对象元数据的结构体，所有资源对象应组合该结构体
...
```
1. GenerateName：如果资源Name为空，k8s为其生成一个名字
2. UID：对象的唯一id
3. ResourceVersion：代表着资源最后一次更新时，后端存储的版本号
4. DeletionTimestamp：不为nil时，说明资源已经开始删除流程
5. DeletionGracePeriodSeconds：资源的删除流程允许的最大时间
6. Finalizers：删除资源时进行的清理步骤，控制器完成某个清理操作后将会移除掉切片中对应的字符串，Finalizers为空时，代表资源可以从后端存储中删除

## 接口
meta包中的Object接口通常称为`metav1.Object`
```go
// k8s.io/apimachinery/pig/apis/meta/v1/meta.go
