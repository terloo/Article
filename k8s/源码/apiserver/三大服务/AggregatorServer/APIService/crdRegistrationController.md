...
crdRegsitrationController会Watch CRD对象并转换成APIService，添加到autoRegistrationController的队列中进行持久化。
...
## handleVersionUpdate 处理CRD的GV，将其转为APIService
```go
// pkg/controlplane/controller/crdregistration/crdregistration_controller.go
func (c *crdRegistrationController) handleVersionUpdate(groupVersion schema.GroupVersion) error {
	...
	for _, crd := range crds {
		...
		for _, version := range crd.Spec.Versions {
            ...
            // 换化为APIService添加到autoRegistryController中进行持久化
